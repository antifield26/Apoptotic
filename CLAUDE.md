...
- **测试**: 172+ tests (0 failures), 10 ignored (E2E + doc-tests)
- **代码量**: ~30,000 行 Rust, 10 crates, edition 2024
- **协议**: Minecraft 26.2 (protocol 776), 73 S2C + 37 C2S Play handler 分支 (37 完整, 0 stub)
- **物品注册**: 1,055 个唯一物品/方块, 0 个重复条目
- **配方**: 1,534 运行时, 480+ source 定义, 0 个 result_item 不匹配 (含旗帜/烟花/锻造纹饰/药箭)
- **附魔**: 42 种注册 (100%), 39 种连线 (93%)
- **效果**: 33 种定义, 27 种连线 (82%) — 含 Speed/Slowness/MiningFatigue/Haste/JumpBoost/Luck/Unluck/ConduitPower/SlowFalling/DolphinGrace/Blindness
- **命令**: 63 commands, @a/@p/@r/@s selectors, OP permissions, /execute, tab completion
- **红石**: 35 组件 (压力板/拌线钩/比较器减法/观察者/hopper 传输/容器填充率)
- **弹射物**: 三叉戟忠诚/Channeling, 火弩烟花爆炸, 弹射物附魔(火焰/力量/冲击/穿透)
- **实体 AI**: ~74 种有独特 AI (81%) — 含 Guardian/IronGolem/SnowGolem/Phantom/Allay/PolarBear/Llama + 7 种类属敌对
- **文档**: README + CONTRIBUTING + PLUGIN_TUTORIAL
- **运维**: /status JSON 端点, benchmark.sh, CI 安全审计
- **Clippy**: 0 warnings
...
├── mc-core/          # 基础 — BlockState, ItemRegistry(1055), Effect(33), Biome(59), constants(91 EntityType)
...
- **物品注册**: 1,055/~2,200 (48%), 较多装饰/特殊方块未注册
- **配方**: 1,534 运行时/~1,700 (90%), 480 source 定义 — 旗帜/烟花/锻造纹饰/药箭已实现
- **C2S 处理器**: 37/54 (69%), 37 完整 + 0 stub — 全部已功能实现, 缺失 JigsawGenerate/StructureBlock/CommandBlock/ChunkBatchResponse 等管理包
- **效果**: 27/33 连线 (82%) — SlowFalling/DolphinGrace/Blindness 已连线, 仅 Nausea/NightVision/Glowing/Darkness 纯客户端
- **实体 AI**: ~74/91 (81%) — Guardian/IronGolem/SnowGolem/Phantom/Allay/PolarBear/Llama 已添加
- **弹射物**: 三叉戟忠诚+Channeling 已实现, 火弩烟花爆炸已实现
- **实体 ID**: 自定义范围 111-132 非官方 vanilla ID
- **RPi 5**: target-cpu=cortex-a76, benchmark 脚本就绪, PGO/BOLT 需手动运行
