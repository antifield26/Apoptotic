...
fn effective_view_distance(server: &ServerRef) -> u8 {
...
async fn handle_play(
    ...
    // ── Play 主循环 ──
    play_loop(io, username, _uuid, server, entity_id, initial_loaded, peer_socket).await;
...
async fn stream_new_chunks(
    ...
    *player_chunk = new_chunk;
    ...
    if !new_chunks.is_empty() {
        debug!(
            "Chunk streaming: ({},{}) → ({},{}) ({} new chunks)",
            old_chunk.x, old_chunk.z, new_chunk.x, new_chunk.z, new_chunks.len(),
        );
...
fn weapon_damage(item_id: u32) -> f32 {
    match item_id {
        // Swords
...
async fn play_loop(
    ...
    loop {
        ...
        match packet_id {
            ...
            // Player Action (0x26) — includes block break
            0x27 => {
                if let Ok((_, payload)) = io.codec().parse_packet_id_and_payload(&frame)
                    && payload.len() >= 4 {
                        let status = payload[0] as i32;
                        let pos_bytes = &payload[1..];
                        if pos_bytes.len() >= 8 {
                            let raw = i64::from_be_bytes(pos_bytes[..8].try_into().unwrap_or([0;8]));
                            let x = (raw >> 38) as i32;
                            // Y is 12-bit signed — sign-extend from bit 11
                            let y = ((raw << 52) >> 52) as i32;
                            let z = (raw >> 12) as i32 & 0x3FFFFFF;
                            // Validate Y is in world bounds (-64..=319)
                            ...
                            if status == 2 {
                                ...
                                if let Some(mut chunk) = server.chunk_store.get_mut(&cp) {
                                    ...
                                    // Recalc lighting + resend modified chunk
                                    ...
                                    let break_particle = Particle {
                                        particle_id: 0, // block break particle
                                        long_distance: false,
                                        x: x as f64 + 0.5, y: y as f64 + 0.5, z: z as f64 + 0.5,
                                        offset_x: 0.2, offset_y: 0.2, offset_z: 0.2,
                                        max_speed: 0.1,
                                        count: 10,
                                    };
                                    let _ = send_packet(io, &break_particle).await;
                                    // Get held item for enchantment checks
                                    ...
                                    let held_enchants = held_item.as_ref()
                                        ...
                                        .unwrap_or_default();
                                    // Unbreaking: (100/(level+1))% chance to NOT consume durability
                                    ...
                                    // Spawn item drop entity if block wasn't air
                                    if !old_block.is_air() {
                                        // Fortune: bonus drops for ores
                                        ...
                                        let _ = send_packet(io, &SpawnEntity {
                                            ...
                                            pitch: 0, yaw: 0, head_yaw: 0,
                                            data: 1,
                                            vel_x: 0, vel_y: 150, vel_z: 0, // small upward velocity
                                        }).await;
                                        // Register for pickup tracking
            ...
            0x16 => {
                match io.codec().decode::<mc_protocol::packets::play::CookieResponse>(&frame) {
                    Ok(cookie) => {
                        debug!("Play cookie from {}: key={}, has_payload={}",
                            username, cookie.key, cookie.payload.is_some());
                    }
            ...
            // Player Input (0x29) — 1.21.5 new movement input packet
            0x29 => {
                // PlayerInput provides continuous input state (forward/back/left/right/jump/sneak/sprint).
                // Discrete sprint/sneak toggles are handled via PlayerCommand (0x1F).
                // Vehicle movement uses this packet's movement vector in vanilla.
                // For now, acknowledge receipt to prevent client timeout.
                if let Ok((_, payload)) = io.codec().parse_packet_id_and_payload(&frame)
                    && payload.len() >= 11 {
                        let flags = payload[0] as u32 | ((payload[1] as u32) << 8) | ((payload[2] as u32) << 16);
                        let sideways = f32::from_le_bytes([payload[3], payload[4], payload[5], payload[6]]);
                        let forward = f32::from_le_bytes([payload[7], payload[8], payload[9], payload[10]]);
                        debug!("PlayerInput from {}: flags=0x{:06X}, move=({:.2}, {:.2})",
            ...
            0x23 => {
                if let Ok((_, payload)) = io.codec().parse_packet_id_and_payload(&frame) {
                    ...
                    // Validate: reject out-of-range trade slots
            ...
            0x10 => {
                // Only OP players can lock difficulty
                let is_op = server.player_manager.get(&_uuid).map(|p| p.is_op).unwrap_or(false);
                if !is_op {
            ...
            0x25 => {
                if let Ok((_, payload)) = io.codec().parse_packet_id_and_payload(&frame) {
                    let (block_id, _) = mc_protocol::codec::read_varint_enum(&payload).unwrap_or((0, 0));
                    let block = mc_core::block::BlockState::new(block_id as u32);
                    let _ = server.player_manager.add_item_to_player(&_uuid, block, 1);
                    debug!("Creative pick block {} for {}", block_id, username);
                }
            }
...
async fn send_chunk_data_cached(
    io: &mut PacketStream,
